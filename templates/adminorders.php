...
<head>
    ...
    <style>
        .inline-data,.orderedby,.deliverby
        {
            margin-top:2%;
        }
        .order
...
<body>
    ...
    ?>
    <!-- Orders start -->
    <h4 class="current-page">Profile > <span class="active">Orders</span></h4>
    <h1 class="order-heading">Orders from customers</h1>
    <div class="orders">
        <?php
            if(isset($_SESSION['email']))
            {
                if($_SESSION['email']=='admin@gmail.com')
                {
                    ...
                    {
                        $statustemp="Order Cancelled";
                        mysqli_stmt_bind_param($stmt,"s",$statustemp);
                        ...
                        while($row = mysqli_fetch_assoc($result))
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    $sql3="SELECT * FROM userdetails where userid=?";
                                        ...
                                        echo "<div class='order'>
                                                ...
                                                <div class='orderdesc'>
                                                    ...
                                                    <div class='status'>$status</div>
                                                        <form method='POST' action='../includes/changestatusinc.php?orderid=$orderid'>
                                                            <select name=status class='changestatus'>
                                                                <option value='Baking Order'>Baking Order</option>
                                                                <option value='Baking Completed'>Baking Complete</option>
                                                                <option value='Out for Delivery'>Out for Delivery</option>
                                                                <option value='Order Delivered'>Order Delivered</option>
                                                                <option value='Order Cancelled'>Cancel Order</option>
                                                            </select>
                                                            <button class='detail-btn' name='changestatus-btn' type='submit'>Change Status</button>
                                                        </form>
                                                    </div>
